...
//        log.info("pageRequestDTO -> {}", service.getList(requestDTO).getDtoList().toString());
    ...
    @RequestMapping("/read")
    public String read(int tno, PageRequestDTO pageRequestDTO, Model model) {
        log.info("todoRead -> read controller");
        UserDTO userDTO = service.getOne(tno);
        model.addAttribute("dto", userDTO);
        model.addAttribute("pageRequestDTO", pageRequestDTO);
        return "admin.memberview.page";
    }
    @RequestMapping( "/modify")
    public String modify(int tno, PageRequestDTO pageRequestDTO, Model model) {
        log.info("todoRead -> read controller");
        UserDTO userDTO = service.getOne(tno);
        model.addAttribute("dto", userDTO);
        model.addAttribute("pageRequestDTO", pageRequestDTO);
    ...
    public String modify(@Valid UserDTO userDTO,
        ...
        if (bindingResult.hasErrors()) {
            log.info("binding error...............");
            ra.addFlashAttribute("errors", bindingResult.getAllErrors());
            ra.addAttribute("tno", userDTO.getTno());
            ra.addAttribute("page", pageRequestDTO.getPage());
            ra.addAttribute("size", pageRequestDTO.getSize());
            return "admin.memberupdate.page"+"?"+pageRequestDTO.getLink();
        }
        log.info(userDTO);
    ...
    public String remove(int tno, RedirectAttributes ra, PageRequestDTO pageRequestDTO) {
        log.info("POST Todo -> remove controller");
        log.info("tno -> {}", tno);
        service.remove(tno);
        ra.addAttribute("page", pageRequestDTO.getPage());
        ra.addAttribute("size", pageRequestDTO.getSize());
        return "redirect:/admin/member/list?"+pageRequestDTO.getLink();
